...
class LiDARViewerHandler(BaseHTTPRequestHandler):
    ...
    def __init__(self, *args, output_dir: Path | None = None, **kwargs):
    ...
    def serve_pointcloud(self, filename: str):
        ...
        try:
            ...
            if not las_path.exists() or las_path.suffix != ".las":
            ...
            with laspy.open(las_path) as las_file:
                ...
                for i in indices:
                    point_dict = {"position": points_centered[i].tolist()}
...
def run_viewer(port: int = 8080, output_dir: Path | None = None, open_browser: bool = True):
